    ...
    Umbraco.Controls.MenuItems = Base.extend(null, {
        ...
        deleteItem: function($treeNode, $menuObj) {
            ...
            if ($treeNode.closest(".recycle-bin").length > 0) {
                if (!confirm("This will permanently delete the item: '" + $treeNode.find("span").text() + "'. Are you sure you want to proceed, this action cannot be un-done?")) return;
            }
            else {
                if (!confirm("Are you sure you want to delete '" + $treeNode.find("span").text() + "'?")) return;
            }            
